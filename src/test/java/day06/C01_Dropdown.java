...
public class C01_Dropdown {
    ...
    @Before    //    Eger test sinifinda birden fazla method olusturulmussa, @Before kullanir
    public void setup(){
...
//       State dropdownindaki varsayilan secili secenegin 'Select a State' oldugunu verify edelim
        WebElement state = driver.findElement(By.xpath("//select[@id='state']"));
        Select stateDropDown= new Select(state);
        String varsayilanText=  stateDropDown.getFirstSelectedOption().getText();// buarada özetle dedikki select a state diye bir butaon var mı
        // önce ona bak sonra da o butonda ne secılmıs onu gör!!!
...
//REUSABLE METHOD NEDİR? Dropdown için tekrar tekrar kullanacağımız bir method oluşturalım...
